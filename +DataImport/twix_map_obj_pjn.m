classdef twix_map_obj_pjn < matlab.mixin.Copyable %handle
...
methods
    % Constructor:
    function this = twix_map_obj_pjn(arg,dataType,fname,version,rstraj)
...
%         end
    ...
    function varargout = subsref(this, S)
        ...
        %Manually fix all these
        if this.dataSize(3) ~=length(this.Lin)
            selRange{1,3} = 1:length(this.Lin);
            selRangeSz(3) = length(this.Lin);
            outSize(3) = length(this.Lin);
        end
...
%         if this.dataSize(3) ~=length(this.Lin)
%             this.dataSize(3) = length(this.Lin);
%             this.sqzSize(3) = lenght(this.Lin);
%         end
        
        tmp = reshape(1:prod(double(this.fullSize(3:end))), this.fullSize(3:end));
        if this.dataSize(3) ~=length(this.Lin)
            tmp = reshape(1:prod(double([length(this.Lin) this.fullSize(4:end)])), [length(this.Lin) this.fullSize(4:end)]);
        end
    ...
    function out = readData(this,mem,cIxToTarg,cIxToRaw,selRange,selRangeSz,outSize)
        ...
        if ~exist('outSize','var')
            selRange{1} = ':';
            selRange{2} = ':';
            if mem ~= this.NLin*this.NPar*this.NSli*this.NAve*this.NPhas*this.NEco*this.NRep*this.NSet
                outSize = [this.dataSize(1:2),this.Nacq];
            else
